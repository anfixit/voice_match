...
2. **Подготовка сервера** - запустите на сервере:
   ```bash
   wget https://raw.githubusercontent.com/anfixit/voice_match/main/server-setup.sh
   chmod +x server-setup.sh
   ./server-setup.sh
   ```

3. **Настройка DNS** для вашего домена (A-записи на IP сервера)

4. **Получение SSL сертификата** через Let's Encrypt (инструкции в DEPLOYMENT.md)
